...
interface SuggestionContextType {
  ...
  error: string | null;
  submitSuggestion: (
    suggestion: Omit<Suggestion, "id" | "date" | "status">
  ) => Promise<void>;
...
const convertSuggestionFromAPI = (
  apiSuggestion: SuggestionResponse
): Suggestion => ({
...
export const SuggestionProvider: React.FC<SuggestionProviderProps> = ({
  children,
}) => {
  ...
  const refreshSuggestions = async () => {
    try {
      ...
      const convertedSuggestions = response.suggestions.map(
        convertSuggestionFromAPI
      );
  ...
  const updateSuggestionStatus = async (
    ...
    try {
      ...
      const response = await suggestionService.updateSuggestionStatus(
        id,
        status
      );
